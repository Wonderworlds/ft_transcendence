...
export class PongController {
	...
	onGetLobbyPong(@Param('id') id: string) {
		const lobby = this.pongService.getLobbyID(id);
		if (!lobby) return {error: "Lobby not found", status: 404};
		if (lobby.status !== GameState.PLAYING) return {error: "Pong not started", lastMatch: {...lobby.getMatchLog()}}
	...
	onMovePlayer(@Param('id') id: string, @Param('pseudo') pseudo: string, @Body() body: InputCLIDto) {
